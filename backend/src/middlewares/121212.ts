...
// Cloudinary 업로드 결과를 저장하기 위한 타입 정의
export interface UploadRequest extends Request {
  cloudinaryResult: {
    secure_url: string; // 추가: 이미지를 표시하기 위한 URL
    public_id: string;
    originalName?: string;
  };
  // 미들웨어 체인에서 폴더명을 전달하기 위한 속성 추가
  uploadFolder?: string;
}

// 스트리밍 업로드 미들웨어 팩토리 함수: 폴더명을 인수로 받습니다.
export const createCloudinaryUploader = (folderName: string) => {
  // 실제 미들웨어 함수
